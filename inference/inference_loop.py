...
for idx,batch in enumerate(train_loader):
    ...
    for j in range(config['inference']['max_loop']):
        ...
        if j==0 and begin < config['inference']['skip_threshold']:
            ...
            skipped = True
            break
        if begin < config['inference']['loop_threshold']:
            print('break with '+str(begin))
            break
        begins.append(begin)
        outs1 = diffusion.sample(model,inimg2,1,begin=begin+1+config['inference']['loop_step_addition']*j)
